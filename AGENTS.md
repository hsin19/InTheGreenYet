...
- **Deploy** — one Cloudflare Worker (SPA assets + API) via GitHub Actions (`deploy.yml` on push to `main`, gated by the shared `check.yml`): `pnpm run build` (the vite-plugin emits the client build + Worker bundle + a `.wrangler/deploy` redirect config) then `wrangler deploy`. `wrangler` is also used for local dev, secrets, and debugging.
